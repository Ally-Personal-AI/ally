...
class FunctionalWorkflowEvidenceError(ValueError):
    """Raised when functional workflow evidence cannot be safely loaded."""
...
class FunctionalWorkflowEvidenceReport(BaseModel):
    """Immutable functional evidence tied to one exact capability report."""
    ...
    def validate_evidence(self) -> FunctionalWorkflowEvidenceReport:
        if self.generated_at.tzinfo is None or self.generated_at.utcoffset() is None:
            raise ValueError(
                "functional workflow report timestamp must include a timezone offset"
            )
        if not self.checks:
            raise ValueError("functional workflow evidence requires at least one check")
        return self

    @property
    def workflow_successful(self) -> bool:
        return all(check.status == "passed" for check in self.checks)

    @property
    def qualified_for_candidate_use(self) -> bool:
        return self.source_validation_successful and self.workflow_successful
...
def build_functional_workflow_report(
    *,
    ...
    """Bind disposable workflow results to one exact capability artifact."""
    ...
    if workflow.model != validation.model:
        raise ValueError("workflow model must match the source validation model")

    return FunctionalWorkflowEvidenceReport(
...
def load_functional_workflow_report(
    ...
    try:
        ...
        return FunctionalWorkflowEvidenceReport.model_validate(raw)
...
def verify_functional_workflow_source(
    ...
    """Verify functional evidence against its exact capability source."""
