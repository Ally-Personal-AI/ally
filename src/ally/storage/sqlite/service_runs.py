"""SQLite persistence for proactive service lifecycle metadata."""
...
class SQLiteServiceCycleRunStore:
    """Persist payload-free lifecycle metadata for proactive service cycles."""
        ...
        with self._database.connect() as connection:
            cursor = connection.execute(
                ...
                SET
                    status = 'interrupted',
                    finished_at = ?,
                    error_class = 'StaleLeaseExpired'
                WHERE status = 'running'
                  AND started_at < ?
                """,
                (finished.isoformat(), cutoff.isoformat()),
            )
        ...
        with self._database.connect() as connection:
            try:
                ...
                raise ServiceRunConflictError(
                    "another proactive service cycle is already running"
                ) from exc
