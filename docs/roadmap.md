...
| Native desktop shell | Daily-use foundation advancing; machine acceptance pending | SwiftUI shell over a bounded local stdio bridge to `AllyApplication`; conversations/chat, memory/knowledge inspection, task detail with exact one-step approvals and failed-step retry, runtime readiness, service health, and attention counts; release packaging and hardware acceptance remain |
