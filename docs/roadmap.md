...
| Native desktop shell | Daily-use foundation advancing; machine acceptance pending | SwiftUI shell over a bounded local stdio bridge to `AllyApplication`; conversations/chat, searchable provenance-aware memory correction/retraction, knowledge detail/search/text ingestion, task detail with exact one-step approvals and failed-step retry, validated-profile catalog/selection, full attention event/delivery history with explicit handled state, modern macOS notification authorization UI, runtime readiness, and service health; signed release packaging and hardware acceptance remain |
