...
| Native desktop shell | Daily-use + release foundation advancing; machine acceptance pending | SwiftUI shell over a bounded local stdio bridge to `AllyApplication`; conversations/chat, searchable provenance-aware memory correction/retraction, knowledge detail/search/text ingestion, task detail with exact one-step approvals and failed-step retry, validated-profile catalog/selection, full attention event/delivery history with explicit handled state, modern macOS notification authorization UI, runtime readiness, service health, stable-ID app-bundle/signing foundation, release-only helper verification, and a CI-verified self-contained frozen desktop bridge; real Developer ID/notarization and hardware acceptance remain |
